...
class PaligemmaTokenizer:
    ...
    def tokenize(self, prompt: str, state: np.ndarray | None = None, tactile: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if state is not None or tactile is not None:
            # This is the Pi05 format, where the state is part of the discrete language input.
            prompt_parts = [f"Task: {cleaned_text}"]

            if state is not None:
                discretized_state = np.digitize(state, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
                state_str = " ".join(map(str, discretized_state))
                prompt_parts.append(f"State: {state_str}")
            
            if tactile is not None:
                flat_tactile = tactile.flatten()
                discretized_tactile = np.digitize(flat_tactile, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
                tactile_str = " ".join(map(str, discretized_tactile))
                prompt_parts.append(f"Tactile: {tactile_str}")
