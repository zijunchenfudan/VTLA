...
import openpi.policies.tactile_policy as tactile_policy
                ...
                return _transforms.Group(
                    inputs=[
                        ...
                        _transforms.TokenizePrompt(
                            ...
                            discrete_tactile_input=model_config.discrete_tactile_input,
                        ),
                        _transforms.PadStatesAndActions(model_config.action_dim),
                    ],
                )
...
@dataclasses.dataclass(frozen=True)
class LeRobotTactileDataConfig(DataConfigFactory):
    # If true, will convert joint dimensions to deltas with respect to the current state before passing to the model.
    # Gripper dimensions will remain in absolute values.
    # use_delta_joint_actions: bool = True
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[tactile_policy.TactileInputs(model_type=model_config.model_type)],
